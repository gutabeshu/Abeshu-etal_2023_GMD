...
def streamrouting(L, S0, F0, ChV, q, area, nday, dt, UM, UP, Sini_byr, wdirr, irrmean, 
    ...
    monthly_demand = np.multiply(wdirr, unit_conversion_demand) #downstream  demand: m^3/s
    mean_demand  = np.multiply(irrmean, unit_conversion_demand)   #downstream mean demand:  m^3/s
    ...
    basinppose = ppose[grdc_us_grids] # all reservoir cells   
    cond_all = np.squeeze(np.array(pd.DataFrame(grdc_us_grids)[basinppose > 0]))
    #print(ppose[cond_all].shape) 
    ...
    for t in range(nt):
        ...
        if ((res_flag==1) & (ppose[grdc_us_grids].max() > 0)): 
            ...
            # error = reservoir_water_balance_error(Qin_resv[cond_all], 
			#                                       Qout_resv[cond_all], 
			# 									  Sini_resv[cond_all], 
			# 									  Sending[cond_all],                                                 
			# 									  dt, 
            ...
            # update the channel storage because F has changed due to reservoirs
            #(UM.dot(F))[Sxn]
            #dSdt_updated = (UM.dot(F))[grdc_us_grids]  #+ erlateral [grdc_us_grids]       
            #S[grdc_us_grids] += dSdt_updated * dt            
        else:
            Favg += F
...
def reservoir_water_balance_error(Qin_, Qout_, Sin_, Sout_, dt, us_grid_basinIdx):
    ...
    if np.max(Res_Wbalance_relative_error) > 1e-6:          
        insx_resw = np.where(Res_Wbalance_relative_error > 1e-6)[0].tolist()
        mm = np.where(Res_Wbalance_relative_error==np.max(Res_Wbalance_relative_error))[0]
        print("Qin: {}".format(Qin[mm]))  
        print("Qout:{}".format(Qout[mm]))  
        print("Sin: {}".format(Sin[mm]))                  
        print("Sout:{}".format(Sout[mm]))  

        error_grid = np.squeeze(us_grid_basinIdx)[insx_resw] + 1
        print("Error: Reservoir water balance violated at Xanthos grid ID: {}".format(error_grid))        
        print("Maximum Error : {}".format(Res_Wbalance_relative_error))
...
def channel_water_balance_error(Qin, Qout, Sin, Sout, dt, us_grid_basinIdx):
    """Computes channel water balance error, acceptable erro is <=1e-6"""    
    Qin  = np.array(Qin[us_grid_basinIdx]) 
    Qout = np.array(Qout[us_grid_basinIdx]) 
    Sin  = np.array(Sin[us_grid_basinIdx]) 
    Sout = np.array(Sout[us_grid_basinIdx])   
    # water balance
    Snorm  = 0.5*(Sout + Sin)
    DSdt_Q = (Qin - Qout)*dt
    DSdt_S = (Sout- Sin)
    ...
    # error
    Ch_Wbalance_relative_error = np.zeros([len(us_grid_basinIdx),])
    Szs = ~(Snorm < 1) 
    Ch_Wbalance_relative_error[Szs] = np.divide(DS_diff[Szs],  Snorm[Szs])         
    if np.max(Ch_Wbalance_relative_error) > 1e-6:          
        insx_chw = np.where(Ch_Wbalance_relative_error > 1e-6)[0].tolist()
        mm = np.where(Ch_Wbalance_relative_error==np.max(Ch_Wbalance_relative_error))[0]        
        print("Qin: {}".format(Qin[mm]))  
        print("Qout:{}".format(Qout[mm]))  
        print("Sin: {}".format(Sin[mm]))                  
        print("Sout:{}".format(Sout[mm]))  

        error_grid = np.squeeze(us_grid_basinIdx)[insx_chw] + 1
        print("Error: Channel water balance violated at Xanthos grid ID : {}".format(error_grid))
        print("Maximum Error : {}".format(np.max(Ch_Wbalance_relative_error)))
...
def flood_control_reservoir(cpa, ppose, qin, Sini_yr, mtifl, alpha, dt):
    ...
    qin_flood = qin[condF]                       #inflow flow:  m^3/s
    cpa_flood = cpa[condF]                       #capacity:  m^3    
    Sbegnining_ofyear = Sini_yr[condF]           #storage at begining of  year:  m^3   

    # Provisional Release : m3 over dt period
    Rprovisional = mtifl_flood.copy() 

    # Final Release 
    Rflood_final = np.copy(Rprovisional)
    # capacity & annual total infow
    Imean = mtifl_flood*365*24*3600
    c = np.divide(cpa_flood, Imean)
...
def reservoir_water_balance(Qin, Qout, Sin,  ppose, cpa, mtifl, alpha, dt):
	...
	# inputs
    Qin_resd = np.array(Qin)
    Qout_resd= np.array(Qout)
    Sin_resd = np.array(Sin)
    cpa_resd = np.array(cpa)
    mtifl_res= np.array(mtifl)

    # final storage and release initialization	
    Nx = Qin_resd.size
    Rfinal = np.array(Qout)    # final release
    ...
    # final storage
    Sfinal = np.array(Stemp.copy())
    
    ## Update for conditions other than -- 25% capacity < S < capacity
    # condition a : storage > capacity
    Sa = (Stemp > (alpha*cpa_resd))
    if Sa.any():
        #Sa = np.where(Stemp > (alpha*cpa_resd))[0]
    ...
    # if Sc.any():   
    # Sc = np.where((Stemp > 0) & (Stemp <= alpha*cpa_resd))[0]     
    # Sfinal[Sc] = Stemp[Sc]
    # Rfinal[Sc] = Rfinal[Sc] 

    return Rfinal,  Sfinal 

# Upstream grids for grdc stations
